...
from python_rako.const import MessageType, CommandType, SCENE_COMMAND_TO_NUMBER
from python_rako.exceptions import RakoDeserialisationException
from python_rako.model import ChannelStatusMessage, SceneStatusMessage, SceneCacheMessage, LevelCacheMessage, Command
...
def deserialise_byte_list(byte_list):
    try:
        ...
        raise RakoDeserialisationException(f'Unsupported UDP message type: {byte_list=}')
    ...
    if message_type == MessageType.STATUS:
        ...
        raise RakoDeserialisationException(f'Unsupported UDP message: {message_type=}, {byte_list=}')
